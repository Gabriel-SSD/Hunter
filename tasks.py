...
# Set up logging
logging.basicConfig(filename='bot_tasks.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
async def af_tickets(bot):
    ...
    if now.hour == 17 and now.minute == 31:
        try:
            logging.info(f"Sending ticket report for 'Awakening Fear' guild at {now}.")
            channel = bot.get_channel(CHANNEL_ID)
            plot_ticket_report("1HE3bh3LRcWVOto5KuGvzQ")
            await channel.send(file=discord.File(fp=FILE))
            logging.info("Ticket report for 'Awakening Fear' sent successfully.")
        except Exception as e:
            logging.error(f"Error sending ticket report for 'Awakening Fear' at {now}: {e}")
...
async def ah_tickets(bot):
    ...
    if now.hour == 22 and now.minute == 31:
        try:
            logging.info(f"Sending ticket report for 'Awakening Hope' guild at {now}.")
            channel = bot.get_channel(CHANNEL_ID)
            ...
            logging.info("Ticket report and missed tickets report for 'Awakening Hope' sent successfully.")
        except Exception as e:
            logging.error(f"Error sending ticket report for 'Awakening Hope' at {now}: {e}")
...
async def af_tickets_missed(bot):
    ...
    if now.weekday() == 6 and now.hour == 17 and now.minute == 32:
        try:
            logging.info(f"Sending missed tickets report for 'Awakening Fear' guild at {now}.")
            channel = bot.get_channel(CHANNEL_ID)
            df = get_tickets_missed("1HE3bh3LRcWVOto5KuGvzQ", '7')
            embed = format_embed(df, "Awakening Fear", '7')
            await channel.send(embed=embed)
            logging.info("Missed tickets report for 'Awakening Fear' sent successfully.")
        except Exception as e:
            logging.error(f"Error sending missed tickets report for 'Awakening Fear' at {now}: {e}")
...
async def ah_tickets_missed(bot):
    ...
    if now.weekday() == 6 and now.hour == 22 and now.minute == 32:
        try:
            logging.info(f"Sending missed tickets report for 'Awakening Hope' guild at {now}.")
            channel = bot.get_channel(CHANNEL_ID)
            df = get_tickets_missed("iO-khl_0TVu64OussT1Y7g", '7')
            embed = format_embed(df, "Awakening Hope", '7')
            await channel.send(embed=embed)
            logging.info("Missed tickets report for 'Awakening Hope' sent successfully.")
        except Exception as e:
            logging.error(f"Error sending missed tickets report for 'Awakening Hope' at {now}: {e}")
